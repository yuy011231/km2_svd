...
class SvdCalculator:
    ...
    def _correction_slide_window(self, slide_window_size: int):
        if slide_window_size > len(self._data_df):
    ...
    def svd(self):
        window_slice_power = self._slice_by_window(self._data_df["power"])
        U, s, V = np.linalg.svd(window_slice_power, full_matrices=True)
        return U, s, V
    
    def get_reproduction_peak_df(self):
        u, s, v = self.svd()
        peak = self._reconstruct_from_windows(self._reproduction_peak(u, s, v), len(self._data_df))
        return pd.DataFrame({"time": self._data_df["time"], "peak": peak})
    
    def get_reproduction_noise_df(self):
        u, s, v = self.svd()
        noise = self._reconstruct_from_windows(self._reproduction_noise(u, s, v), len(self._data_df))
        return pd.DataFrame({"time": self._data_df["time"], "noise": noise})
